...
bool cycle_checker(int start, int end) 
{
    // case of 2 candidates in a cycle:
    ...
    for (int j = 0; j < candidate_count; j++) 
    {
        if ((locked[j][start]) == true)
        {
            if (cycle_checker(j, end) == true)
            {
                return true;
            }
        }
...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        if (cycle_checker(pairs[i].winner, pairs[i].loser) == false)
        {
